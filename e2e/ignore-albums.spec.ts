...
test.describe("Ignore Albums", () => {
    ...
    if (count > 0) {
      ...
      if (albumCount > 0) {
        ...
        for (let i = 0; i < albumCount; i++) {
          ...
          const ownershipStatus = albumCard.locator(
  ...
  test("should ignore album and update display", async ({ page }) => {
    ...
    expect(
      count,
      "Should have at least one artist to test with",
    ).toBeGreaterThan(0);
    ...
    expect(
      initialAlbumCount,
      "Should have at least one album to test with",
    ).toBeGreaterThan(0);
    ...
    expect(
      targetAlbumCard,
      "Should find at least one unowned album to ignore",
    ).toBeTruthy();
    ...
    // Click ignore button
    const ignoreButton = targetAlbumCard.locator(
      '[data-testid="ignore-album-button"]',
    );
  ...
  test("should show ignored albums when filter is enabled", async ({
    page,
  }) => {
    ...
    if (count > 0) {
      ...
      if (await filterControls.isVisible()) {
        ...
        const ignoredIndicators = page.locator(
          '[data-testid="ignored-indicator"]',
        );
        ...
        if (ignoredCount > 0) {
          ...
          const unignoreButtons = page.locator(
    ...
    if (count > 0) {
      ...
      if (await filterControls.isVisible()) {
        ...
        if (ignoredCount > 0) {
          ...
          const firstIgnoredAlbum = ignoredIndicators
            .first()
            .locator("..")
            .locator("..");

          // Get the album title
          const albumTitle = firstIgnoredAlbum.locator(
            '[data-testid="album-title"]',
          );
          const titleText = await albumTitle.textContent();

          // Click un-ignore button
          const unignoreButton = firstIgnoredAlbum.locator(
            '[data-testid="unignore-album-button"]',
          );
          ...
          if (await filterControls.isVisible()) {
            ...
            const stillIgnored = await page
              ...
              .filter({
                has: page.locator('[data-testid="ignored-indicator"]'),
              })
              .filter({ hasText: titleText || "" })
              .count();
  ...
  test("should update artist statistics when album is ignored", async ({
    page,
  }) => {
    ...
    if (count > 0) {
      ...
      const initialOwnedText = await firstArtistCard
        .locator('[data-testid="owned-count"]')
        .textContent();
      const initialTotalText = await firstArtistCard
      ...
      const ownedAlbumsText = await page
        .locator("text=Albums Owned")
        .locator("..")
        .locator(".text-2xl")
        .textContent();
      ...
      if (targetAlbumCard) {
        ...
        const updatedOwnedText = await page
          .locator("text=Albums Owned")
          .locator("..")
          .locator(".text-2xl")
          .textContent();
  ...
  // T031: Toggle view test - verify ignored albums appear when filter is enabled
  test("T031: should toggle view to show and hide ignored albums", async ({
    page,
  }) => {
    // Navigate to collection
    await page.click("text=Collection");
    ...
    // First, ensure we have at least one ignored album by ignoring one
    const albumCards = page.locator('[data-testid="album-card"]');
    const initialAlbumCount = await albumCards.count();

    if (initialAlbumCount > 0) {
      // Find an unowned, non-ignored album to ignore
      let targetAlbumCard = null;
      let albumTitle = "";
      ...
      for (let i = 0; i < initialAlbumCount; i++) {
        ...
        const isIgnored = (await ignoredIndicator.count()) > 0;

        if (statusText && statusText !== "Owned" && !isIgnored) {
          targetAlbumCard = albumCard;
          const titleElement = albumCard.locator('[data-testid="album-title"]');
          albumTitle = (await titleElement.textContent()) || "";
          break;
        }
      }
      ...
      if (targetAlbumCard) {
        ...
        // Verify album is hidden in default view
        const visibleAlbumsWithTitle = albumCards.filter({
          hasText: albumTitle,
        });
        const visibleCount = await visibleAlbumsWithTitle.count();
        expect(
          visibleCount,
          "Ignored album should be hidden in default view",
        ).toBe(0);

        // Now test the toggle functionality
        // Find and check the "Show Ignored Albums" checkbox
        const checkbox = page
          .locator('input[type="checkbox"]')
          .filter({ hasText: /Show Ignored Albums/i })
          .or(
            page
              .locator('label:has-text("Show Ignored Albums")')
              .locator('input[type="checkbox"]'),
          );
        ...
        // Verify the ignored album now appears
        const albumCardsAfterToggle = page.locator(
          '[data-testid="album-card"]',
        );
        const albumsWithTitleAfterToggle = albumCardsAfterToggle.filter({
          hasText: albumTitle,
        });
        const countAfterToggle = await albumsWithTitleAfterToggle.count();
        expect(
          countAfterToggle,
          "Ignored album should be visible when filter is enabled",
        ).toBeGreaterThan(0);

        // Verify the album has the ignored indicator
        const ignoredAlbum = albumsWithTitleAfterToggle.first();
        const ignoredIndicator = ignoredAlbum.locator(
          '[data-testid="ignored-indicator"]',
        );
        await expect(ignoredIndicator).toBeVisible();
        await expect(ignoredIndicator).toHaveText("Ignored");

        // Toggle back to hide ignored albums
        await checkbox.uncheck();
        await expect(checkbox).not.toBeChecked();
        await page.waitForTimeout(500);

        // Verify album is hidden again
        const visibleAfterToggleOff = albumCards.filter({
          hasText: albumTitle,
        });
        const countAfterToggleOff = await visibleAfterToggleOff.count();
        expect(
          countAfterToggleOff,
          "Ignored album should be hidden when filter is disabled",
        ).toBe(0);
      }
    }
  });

  // T032: Un-ignore test - verify album can be restored to normal view
  test("T032: should un-ignore an album and restore to normal view", async ({
    page,
  }) => {
    // Navigate to collection
    await page.click("text=Collection");
    ...
    if (initialAlbumCount > 0) {
      ...
      if (targetAlbumCard) {
        ...
        // Find the ignored album
        const albumCardsWithFilter = page.locator('[data-testid="album-card"]');
        const ignoredAlbumCards = albumCardsWithFilter.filter({
          hasText: albumTitle,
        });
        expect(
          await ignoredAlbumCards.count(),
          "Should find the ignored album",
        ).toBeGreaterThan(0);

        const ignoredAlbum = ignoredAlbumCards.first();

        // Verify it has the ignored indicator
        const ignoredIndicator = ignoredAlbum.locator(
          '[data-testid="ignored-indicator"]',
        );
        await expect(ignoredIndicator).toBeVisible();

        // Verify it has visual indicators (opacity and strikethrough)
        await expect(ignoredAlbum).toHaveClass(/opacity-50/);
        const titleElement = ignoredAlbum.locator(
          '[data-testid="album-title"]',
        );
        await expect(titleElement).toHaveClass(/line-through/);

        // Click un-ignore button
        const unignoreButton = ignoredAlbum.locator(
          '[data-testid="unignore-album-button"]',
        );
        ...
        // Verify the album no longer has the ignored indicator
        const updatedAlbumCards = page.locator('[data-testid="album-card"]');
        const restoredAlbumCards = updatedAlbumCards.filter({
          hasText: albumTitle,
        });
        const restoredAlbum = restoredAlbumCards.first();

        const stillHasIgnoredIndicator = restoredAlbum.locator(
          '[data-testid="ignored-indicator"]',
        );
        await expect(stillHasIgnoredIndicator).not.toBeVisible();

        // Verify it no longer has opacity-50 class
        await expect(restoredAlbum).not.toHaveClass(/opacity-50/);

        // Verify title no longer has line-through
        const restoredTitle = restoredAlbum.locator(
          '[data-testid="album-title"]',
        );
        await expect(restoredTitle).not.toHaveClass(/line-through/);

        // Disable showing ignored albums
        await checkbox.uncheck();
        await page.waitForTimeout(500);

        // Verify the album is still visible in normal view (because it's no longer ignored)
        const normalViewAlbums = page.locator('[data-testid="album-card"]');
        const visibleRestoredAlbums = normalViewAlbums.filter({
          hasText: albumTitle,
        });
        expect(
          await visibleRestoredAlbums.count(),
          "Un-ignored album should be visible in normal view",
        ).toBeGreaterThan(0);

        // Verify it has the ignore button again (not the un-ignore button)
        const restoredAlbumInNormalView = visibleRestoredAlbums.first();
        const ignoreButtonAgain = restoredAlbumInNormalView.locator(
          '[data-testid="ignore-album-button"]',
        );
        await expect(ignoreButtonAgain).toBeVisible();

        const unignoreButtonShouldNotExist = restoredAlbumInNormalView.locator(
          '[data-testid="unignore-album-button"]',
        );
        await expect(unignoreButtonShouldNotExist).not.toBeVisible();
      }
    }
  });
});

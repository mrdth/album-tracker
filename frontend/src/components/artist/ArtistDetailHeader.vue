...
async function handleDeleteArtist() {
  const albumCount = props.artist.total_albums || 0
  const message =
    albumCount > 0
      ? `Are you sure you want to delete "${props.artist.name}" and all ${albumCount} associated album${albumCount === 1 ? '' : 's'}? This action cannot be undone.`
      : `Are you sure you want to delete "${props.artist.name}"? This action cannot be undone.`
  ...
  try {
    ...
    await api.deleteArtist(props.artist.id)
        ...
        <div class="flex flex-col items-end gap-2">
          <div class="flex items-center gap-2">
            ...
            <button
              ...
              class="px-3 py-2 text-sm font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-red-500 disabled:opacity-50 disabled:cursor-not-allowed"

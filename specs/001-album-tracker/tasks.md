...
- [X] T032 [P] [US1] Create Artist model in backend/src/models/Artist.ts (MBID, name, sort_name, disambiguation, linked_folder_path)
- [X] T033 [P] [US1] Create Album model in backend/src/models/Album.ts (MBID, title, release_year, ownership_status, match_confidence, is_manual_override)
- [X] T034 [US1] Create ArtistRepository in backend/src/repositories/ArtistRepository.ts (create, findByMbid, findById, list with stats)
- [X] T035 [US1] Create AlbumRepository in backend/src/repositories/AlbumRepository.ts (bulkCreate, findByArtistId, updateOwnership)
- [X] T036 [US1] Implement artist search in MusicBrainzService.searchArtists() (query MusicBrainz API, handle rate limits)
- [X] T037 [US1] Implement discography import in MusicBrainzService.fetchReleaseGroups() (fetch albums, filter type: Album only)
- [X] T038 [US1] Implement GET /api/artists/search route in backend/src/api/routes/artists.ts (call MusicBrainzService, return sorted results)
- [X] T039 [US1] Implement POST /api/artists route in backend/src/api/routes/artists.ts (import artist + albums, handle duplicates with 409)
- [X] T040 [US1] Implement GET /api/artists/:artistId route in backend/src/api/routes/artists.ts (return artist with all albums)
- [X] T041 [US1] Implement GET /api/artists route in backend/src/api/routes/artists.ts (Collection Overview - all artists with stats)
- [X] T042 [P] [US1] Create ArtistSearchForm component in frontend/src/components/search/ArtistSearchForm.vue (input, search button, loading state)
- [X] T043 [P] [US1] Create ArtistSearchResults component in frontend/src/components/search/ArtistSearchResults.vue (result cards, import buttons)
- [X] T044 [P] [US1] Create useArtistSearch composable in frontend/src/composables/useArtistSearch.ts (reactive search state, API calls)
- [X] T045 [US1] Create HomePage in frontend/src/pages/HomePage.vue (integrate search form + results)
- [X] T046 [P] [US1] Create ArtistDetailHeader component in frontend/src/components/artist/ArtistDetailHeader.vue (artist name, stats, disambiguation)
- [X] T047 [P] [US1] Create AlbumCard component in frontend/src/components/artist/AlbumCard.vue (title, year, ownership status badge)
- [X] T048 [P] [US1] Create AlbumGrid component in frontend/src/components/artist/AlbumGrid.vue (grid layout with AlbumCard instances)
- [X] T049 [US1] Create ArtistDetailPage in frontend/src/pages/ArtistDetailPage.vue (header + album grid)
- [X] T050 [US1] Add error handling and user-friendly error messages to all US1 components (95% self-serve resolution per SC-010)
- [X] T051 [US1] Add loading states and ARIA live regions to all US1 components (screen reader announcements)
